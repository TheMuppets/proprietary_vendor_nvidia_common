...
def sign_files_in_listmeta(p_key, internal):
    ...
    pkh, _, _, _= get_pkh_args(internal)
...
def is_key_list(filelistname, p_key, verbose):
    try:
        ...
        for entry in root.findall('entry'):
            ...
            if is_PKC_pubkey(key, p_key, pub_file, mont=None, exp=None) is True \
                or is_PKC_key(key, p_key, pub_file, mont=None, exp=None) is True:
...
def is_PKC_key(keyfilename, p_key, pkh, mont, exp):
    ...
    if pkh and mont:
        if is_hsm():
            ...
            return get_rsa_mod_hsm(p_key, pkh, exp) and get_rsa_mont_hsm(p_key, mont)

        command.extend(['--isPkcKey', keyfilename, pkh, mont])
    elif pkh:
        if is_hsm():
            get_rsa_mod_hsm = import_function('get_rsa_mod_hsm')
            return get_rsa_mod_hsm(p_key, pkh, exp)
...
def is_PKC_pubkey(keyfilename, p_key, pkh, mont, exp):
    ...
    if pkh and mont:
        ...
        if is_hsm():
            get_rsa_mod_from_pubkey_hsm = import_function('get_rsa_mod_from_pubkey_hsm')
            return get_rsa_mod_from_pubkey_hsm(p_key, pkh, pub_expf=exp)
...
def do_kdf_params_oem(dk, params, kdf_list, p_key):
    ...
    kdk_ctx['Msg'] = get_composed_msg(kdk_ctx['Label'], '00', L, is_str)
    bl_kdk_ctx = {}
    fw_kdk_ctx = {}
    gp_kdk_ctx = {}
    gpto_kdk_ctx = {}
    ...
    while kdk_to_use in sbk_list and (count>0):
        ...
        # Check if sbk_fw_kdk is defined for this dk
            ...
            if p_key.kdf.deviceid.is_t234() == True:
                ...
                fw_kdk_ctx = {
                    'KDK'   : kdk_to_use,
                    'Label' : fw_kdk_params['Label'],
                    'Context' : p_key.kdf.fw_label.get_strbuf(),
                }
                fw_kdk_ctx['Msg'] = get_composed_msg(fw_kdk_ctx['Label'], fw_kdk_ctx['Context'], L, False)
    ...
    return [None, None,  None, sbk_keystr, None,
        fw_kdk_ctx['Msg'], bl_kdk_ctx['Msg'], tz_kdk_ctx['Msg'], gp_kdk_ctx['Msg'], kdk_ctx['Msg'], dk_ctx['Msg']], \
        dk_ctx['Msg']
...
def do_kdf_params_oem_t234(dk, params, kdf_list, p_key):
    ...
    return [None, None, None, sbk_keystr, None, None,
...
def create_keystore(params, p_key, create_dk = False, user_kdk = False, dk_ctx = None):
    ...
    for dk_id in dk_list:
        ...
        while int_kdk in ['SBK_NVMB_KDK', 'SBK_HPSE_NVMB_KDK', 'SBK_SB_NVMB_KDK', 'SBK_TZ_KDK', 'SBK_GP_TOSB_KDK', 'SBK_GP_KDK', 'SBK_FW_KDK']:
            if int_kdk in ['SBK_NVMB_KDK', 'SBK_HPSE_NVMB_KDK', 'SBK_SB_NVMB_KDK']:
                if p_key.kdf.deviceid.is_t234():
                    ...
                    context = p_key.kdf.bl_label.get_strbuf()
                    is_blkdk = True
            elif int_kdk == 'SBK_FW_KDK':
                if p_key.kdf.deviceid.is_t234():
                    label = p_key.kdf.fw_label.get_strbuf()
                    context = ''
                else:
                    label = literal_to_hex(params['KDK'][int_kdk]['Label'])
                    context = p_key.kdf.fw_label.get_strbuf()
...
def do_kdf_oem(params_slist, params, kdf_list, p_key, blockSize, dk_ctx = None):
    ...
    # to write to file
    # order: sizes then data for: deckdk_kdkkdd, deckdk_iv, deckdk_aad, sbk_plain, deckdk_msg, fw_kdk_msg, bl_kdk_msg,
    #        tzkdk_msg, gpkdk_msg, kdk_msg, dk_msg, iv, aad, tag, src, flag, result_name
