...
-------
    1) if blob-type = update then entry tuple format is:
       ...
       example: BUP_generator.py -t update -e "cboot.bin EBT 2 0 <spec_info>"
...
def parse_args():
    ...
    # The accessory part (8 Bytes) in blob header is optional, can be
    # present or not as per header offset. Not used.
    parser.add_argument('-r', dest='accessory',
                        help=textwrap.dedent('''\
                             header accessory binary
                             not used, hence invalid now
                             ''')
                       )
