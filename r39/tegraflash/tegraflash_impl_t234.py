#
# SPDX-FileCopyrightText: Copyright (c) 2014-2026, NVIDIA Corporation.  All Rights Reserved.
...
class TFlashT23x_Base(object):
    ...
    def _tegrabct_extend_parse_error_mode(self, command):
        """Pass tegrabct_v2 parse behavior when creating or modifying BCTs."""
    ...
    def tegraflash_parse_partitionlayout(self, dce_comb=True):
        ...
        if values['--no_pva']:
            self.tegraflash_update_mb2_bct_common_dtsi()
    ...
    def tegraflash_fill_mb1_storage_info(self):
        ...
        if self.tegraparser_values['--pt'] is not None:
            ...
            self._tegrabct_extend_parse_error_mode(command)
            run_command(command)
    ...
    def tegraflash_update_mb2_bct_common_dtsi(self):
        if values['--no_pva'] is None:
            return

        if values.get('--no_pva') == '1':
            filename = 'tegra234-mb2-bct-common.dtsi'
            property_name = 'enable_pva_fw'
            new_value = '<0>'

            if not os.path.exists(filename):
                raise tegraflash_exception('Error: Can not find tegra234-mb2-bct-common.dtsi file')

            with open(filename, 'r') as file:
                content = file.read()

            pattern = r'{}\s*=\s*<\d+>;'.format(property_name)
            updated_content = re.sub(pattern, '{} = {};'.format(property_name, new_value), content)
    ...
    def tegraflash_oem_sign_file(self, in_file, magic_id, partition_type=None):
        ...
        ecc521 = ElementTree.SubElement(child, 'ec521')
        ecc521.set('signature', filename + '.sig')
        ecc521.set('signed_file', filename + '.signed')
    ...
    def tegraflash_oem_enc_and_sign_file(self, in_file, magic_id, partition_type=None):
        ...
        is_aligned = False
        algo_list = {'pkc':'oem-rsa', 'ec':'oem-ecc', 'ec521':'oem-ecc521', 'eddsa':'oem-eddsa', 'xmss':'oem-xmss'}
    ...
    def tegraflash_dumpcustinfo(self, dump_args):
        ...
        if os.path.exists(file_path):
            info_print('Renaming %s to %s' %(file_path, file_path + '.sav'))
            shutil.move(file_path, file_path + '.sav')
