...
bin_dir = os.path.dirname(os.path.realpath(__file__)) + os.sep
pid = str(os.getpid())
...
def get_pkh_args(internal):
    ...
    return pk, phk, mode, exp

'''
Prints the public modulus and exponent, this is only for rsa key
'''
def print_ist_pcp(arr_pcp, arr_exp, oem_val):
    arr_str = hex_to_str(arr_pcp)
    n = len(arr_str)
    ...
    print('dts format: \n')
    print('ist_rsa3k_pub_param= [',  end=' ')
    for i in range(0, int(n/2)):
        print(hex_to_str((bytearray(arr_pcp[i:(i+1)]))), end=' ')

    arr_str = hex_to_str(arr_exp)
    n = len(arr_str)
    if n % 4 != 0:
        return
    for i in range(0, int(n/2)):
        print(hex_to_str((bytearray(arr_exp[i:(i+1)]))), end=' ')

    print("]; ist_oem_cust_value= [", oem_val, "];")
